...
// }
function randomColor() {
    // Generate a shuffled array of unique values for R, G, B
    let values = [Math.floor(Math.random() * 256), Math.floor(Math.random() * 256), Math.floor(Math.random() * 256)];
    while (values[0] === values[1] || values[1] === values[2] || values[0] === values[2]) {
        // Regenerate values if any two are the same
    ...
    for (var i = 0; i < 6; i++) {
        squares[i].addEventListener("click", function() {
            ...
            if (clickedColor === pickedColor) {
                ...
                showFooter(); // Show the footer with the picked color
            } else {
